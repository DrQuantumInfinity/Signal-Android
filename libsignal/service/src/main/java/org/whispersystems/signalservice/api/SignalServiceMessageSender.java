...
import org.whispersystems.signalservice.internal.push.SignalServiceProtos.BACommandMessage;
...
public class SignalServiceMessageSender {
    ...
    public void sendReceipt(SignalServiceAddress recipient,
        ...
        sendMessage(recipient, getTargetUnidentifiedAccess(unidentifiedAccess), message.getWhen(), content, false, null, AresId.ARESID_CMD);
    }
    ...
    public void sendTyping(SignalServiceAddress recipient,
        ...
        sendMessage(recipient, getTargetUnidentifiedAccess(unidentifiedAccess), message.getTimestamp(), content, true, null, AresId.TEXT);
    }
    ...
    public void sendTyping(List<SignalServiceAddress> recipients,
            ...
            throws IOException {
        byte[] content = createTypingContent(message);
        sendMessage(recipients, getTargetUnidentifiedAccess(unidentifiedAccess), message.getTimestamp(), content, true, cancelationSignal, AresId.TEXT);
    }
    ...
    public SendMessageResult sendMessage(SignalServiceAddress recipient,
        ...
        SendMessageResult result = sendMessage(recipient, getTargetUnidentifiedAccess(unidentifiedAccess), timestamp, content, false, null, AresId.TEXT);
        ...
        if (result.getSuccess() != null && result.getSuccess().isNeedsSync()) {
            ...
            sendMessage(localAddress, Optional.<UnidentifiedAccess>absent(), timestamp, syncMessage, false, null, AresId.TEXT);
        }
    ...
    public List<SendMessageResult> sendMessage(List<SignalServiceAddress> recipients,
        ...
        byte[] content = createMessageContent(message);
        long timestamp = message.getTimestamp();
        List<SendMessageResult> results = sendMessage(recipients, getTargetUnidentifiedAccess(unidentifiedAccess), timestamp, content, false, null, AresId.TEXT);
    ...
    public void sendMessage(SignalServiceSyncMessage message, Optional<UnidentifiedAccessPair> unidentifiedAccess)
        ...
        sendMessage(localAddress, Optional.<UnidentifiedAccess>absent(), timestamp, content, false, null, AresId.TEXT);
    }
    ...
    private void sendMessage(VerifiedMessage message, Optional<UnidentifiedAccessPair> unidentifiedAccess)
        ...
        SendMessageResult result = sendMessage(message.getDestination(), getTargetUnidentifiedAccess(unidentifiedAccess), message.getTimestamp(), content, false, null, AresId.TEXT);

        if (result.getSuccess().isNeedsSync()) {
            byte[] syncMessage = createMultiDeviceVerifiedContent(message, nullMessage.toByteArray());
            sendMessage(localAddress, Optional.<UnidentifiedAccess>absent(), message.getTimestamp(), syncMessage, false, null, AresId.TEXT);
        }
    }
    ...
    //  public static byte[] createLocationRequestContent(long messageTimestamp) {
//    Content.Builder container = Content.newBuilder();
//    LocationMessage.Builder builder = LocationMessage.newBuilder();
//    builder.setTimestamp(messageTimestamp);
//    builder.setType(LocationMessage.Type.REQUEST);
//    return container.setLocationMessage(builder).build().toByteArray();
//  }
//
//  private byte[] createLocationRefusalContent(long messageTimestamp) {
//    Content.Builder container = Content.newBuilder();
//    LocationMessage.Builder builder = LocationMessage.newBuilder();
//    builder.setTimestamp(messageTimestamp);
//    builder.setType(LocationMessage.Type.REFUSAL);
//    return container.setLocationMessage(builder).build().toByteArray();
//  }//  private static BAGeoMessage createGeoMessageContent(GeoMessage message) {
...
//    builder.setCekUri(message.getCekUri());
//    builder.setPayloadLength(message.getPayloadLength());
//    builder.setPayloadData(ByteString.copyFrom(message.getPayloadData()));
//    //TODO: do we need payloadLength?
//    return builder.build();
//  }
    private byte[] createGeoMessageContent(SignalServiceGeoMessage message) {
        Content.Builder container = Content.newBuilder();
        BAGeoMessage.Builder builder = BAGeoMessage.newBuilder();
        builder.setName(message.getName());
        builder.setNotBeforeUtc(message.getNotBeforeUtc());
        ...
        builder.setAuthorizationUri(message.getAuthorizationUri());
        builder.setCekUri(message.getCekUri());
        //TODO: why do we need payloadLength?
        builder.setPayloadLength(message.getPayloadLength());
        builder.setPayloadData(ByteString.copyFrom(message.getPayloadData()));
    ...
    private static BACommandMessage.LocationMessage createProtoLocationMessage(LocationMessage message) {
        ...
        builder.setHorizontalAccuracy(message.getHorizontalAccuracy());
        builder.setVerticalAccuracy(message.getVerticalAccuracy());
        return builder.build();
    }
...
//    builder.setTimestamp(messageTimestamp);
        ...
        builder.setLocationMessage(createProtoLocationMessage(locationMessage));
        return container.setBaCommandMessage(builder).build().toByteArray();
    }

    private byte[] createBaPullCommandContent(SignalServiceBaCommandMessage.PullReq pullReq) {
        Content.Builder container = Content.newBuilder();
        BACommandMessage.Builder builder = BACommandMessage.newBuilder();
//    builder.setTimestamp(messageTimestamp);
        builder.setPullReq(BACommandMessage.PullReq.valueOf(pullReq.name()));
        builder.setType(BACommandMessage.Type.PULL);
        return container.setBaCommandMessage(builder).build().toByteArray();
    }

    private byte[] createBaNackCommandContent(SignalServiceBaCommandMessage.PullReq pullReq,
                                              SignalServiceBaCommandMessage.Status status) {
        Content.Builder container = Content.newBuilder();
        BACommandMessage.Builder builder = BACommandMessage.newBuilder();
//    builder.setTimestamp(messageTimestamp);
        builder.setPullReq(BACommandMessage.PullReq.valueOf(pullReq.name()));
        builder.setStatus(BACommandMessage.Status.valueOf(status.name()));
        builder.setType(BACommandMessage.Type.PUSH);
        return container.setBaCommandMessage(builder).build().toByteArray();
    }
    ...
    private List<SendMessageResult> sendMessage(List<SignalServiceAddress> recipients,
        ...
        while (recipientIterator.hasNext()) {
            ...
            futureResults.add(executor.submit(() -> sendMessage(recipient, access, timestamp, content, online, cancelationSignal, aresId)));
        }
    ...
    private SendMessageResult sendMessage(SignalServiceAddress recipient,
        ...
        for (int i = 0; i < RETRY_COUNT; i++) {
            ...
            try {
                OutgoingPushMessageList messages = getEncryptedMessages(socket, recipient, unidentifiedAccess, timestamp, content, aresId, online);
    ...
    private OutgoingPushMessageList getEncryptedMessages(PushServiceSocket socket,
        ...
        if (!recipient.matches(localAddress) || unidentifiedAccess.isPresent()) {
            messages.add(getEncryptedMessage(socket, recipient, unidentifiedAccess, SignalServiceAddress.DEFAULT_DEVICE_ID, plaintext, aresId));
        }
        ...
        for (int deviceId : store.getSubDeviceSessions(recipient.getIdentifier())) {
            if (store.containsSession(new SignalProtocolAddress(recipient.getIdentifier(), deviceId))) {
                messages.add(getEncryptedMessage(socket, recipient, unidentifiedAccess, deviceId, plaintext, aresId));
            }
        }
    ...
    private OutgoingPushMessage getEncryptedMessage(PushServiceSocket socket,
        ...
        try {
            return cipher.encrypt(signalProtocolAddress, unidentifiedAccess, plaintext, aresId);
        } catch (org.whispersystems.libsignal.UntrustedIdentityException e) {
