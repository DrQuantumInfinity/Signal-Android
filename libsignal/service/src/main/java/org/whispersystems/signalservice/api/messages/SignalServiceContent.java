...
public final class SignalServiceContent {
    ...
    private static SignalServiceReceiptMessage createReceiptMessage(SignalServiceMetadata metadata, SignalServiceProtos.ReceiptMessage content) {
        ...
        if (content.getType() == SignalServiceProtos.ReceiptMessage.Type.DELIVERY)
            type = SignalServiceReceiptMessage.Type.DELIVERY;
        else if (content.getType() == SignalServiceProtos.ReceiptMessage.Type.READ)
    ...
    private static SignalServiceTypingMessage createTypingMessage(SignalServiceMetadata metadata, SignalServiceProtos.TypingMessage content) throws ProtocolInvalidMessageException {
        ...
        if (content.getAction() == SignalServiceProtos.TypingMessage.Action.STARTED)
            action = SignalServiceTypingMessage.Action.STARTED;
        else if (content.getAction() == SignalServiceProtos.TypingMessage.Action.STOPPED)
...
//TODO fully implement
            ...
            case PULL:
                switch (content.getPullReq()) {
                    ...
                    case LOCATION:
                        ...
                        break;
                    case ZMQ_TOPIC:
                        signalServiceBaCommandMessage = SignalServiceBaCommandMessage.forZmq(content.getTimestamp(), createZmqTopicMessage(content.getZmqTopicRegistrationMessage()));
                        break;
                }
                break;
            case PUSH:
    ...
    private static ZmqTopicMessage createZmqTopicMessage(SignalServiceProtos.BACommandMessage.ZmqTopicRegistrationMessage content) throws ProtocolInvalidMessageException {
        return new ZmqTopicMessage(content.getTopic());
    }
